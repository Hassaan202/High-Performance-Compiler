...
# Compiler flags
CPPFLAGS = -I$(HOMEBREW_LLVM)/include
...
MLIR_LIBS = -lMLIR -lMLIRExecutionEngine -lMLIRLLVMDialect -lMLIRLLVMIRTransforms \
            ...
            -lMLIROpenMPDialect -lMLIRFuncDialect
...
# Link to executable
$(BIN_FILE): $(OBJ_FILE)
	clang -o $(BIN_FILE) $(OBJ_FILE)

# Build and run
run: $(EXECUTABLE) $(TEST_FILE)
	./$(EXECUTABLE) < $(TEST_FILE) > $(IR_FILE)
	$(LLC) -filetype=obj $(IR_FILE) -o $(OBJ_FILE)
	clang $(OBJ_FILE) -o $(BIN_FILE) 
	./$(BIN_FILE)

# Clean up
clean:
	rm -f $(LEX_C) $(PARSER_C) $(PARSER_H) $(EXECUTABLE) $(IR_FILE) $(OPT_IR_FILE) \
	  $(ASM_FILE) $(OBJ_FILE) $(BIN_FILE)
